...
const MeetingsSection = () => {
  ...
  const formatDate = (dateString: string) => {
    ...
    return format(date, 'MMMM d, yyyy', { locale: locales[language as keyof typeof locales] });
  };
    ...
    >
      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className="max-w-4xl mx-auto">
          <div className={`relative ${isRTL ? 'pr-8 md:pr-12' : 'pl-8 md:pl-12'}`}>
            {/* Timeline Line */}
            <div className={`absolute top-0 bottom-0 w-px bg-gradient-to-b from-accent via-primary to-accent/30 ${isRTL ? 'right-0' : 'left-0'}`} />
              ...
              {meetings.map((meeting, index) => (
                ...
                >
                  <div className="bg-card rounded-2xl p-6 md:p-8 shadow-elegant hover:shadow-lg transition-all duration-300 border border-border/50 hover:border-accent/30">
                    {/* Date Badge */}
                    <div className={`flex items-center gap-2 text-accent text-sm font-medium mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
