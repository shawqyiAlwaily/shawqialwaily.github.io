...
const BiographySection = () => {
  ...
  const socialLinks = [{
    ...
    icon: Youtube,
    url: personalInfo.socialLinks.youtube,
    label: 'YouTube'
  }, {
    icon: Instagram,
  ...
  return <section id="biography" ref={sectionRef} className="relative py-24 md:py-32 bg-background overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        ...
        <div className={`grid lg:grid-cols-2 gap-12 lg:gap-20 items-center ${isRTL ? 'lg:grid-flow-dense' : ''}`}>
          {/* Image Column */}
          <div className={`relative ${isVisible ? 'animate-slide-in-left' : 'opacity-0'} ${isRTL ? 'lg:col-start-2' : ''}`} style={{
          animationDelay: '0.2s'
        }}>
            <div className="relative aspect-[3/4] rounded-2xl overflow-hidden shadow-book">
              <img alt="Portrait" className="w-full h-full object-cover" src="/lovable-uploads/5671b763-2a42-472c-9ee1-743fb1bc8872.jpg" />
              {/* Decorative Frame */}
          ...
          animationDelay: '0.4s'
            ...
            <div className="pt-8 border-t border-border">
              ...
              <div className="flex gap-4">
                {socialLinks.map(social => <a key={social.label} href={social.url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full bg-primary/5 hover:bg-accent hover:text-accent-foreground flex items-center justify-center text-foreground/70 transition-all duration-300 hover:shadow-glow" aria-label={social.label}>
